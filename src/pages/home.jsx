...
import {
  Phone, Mail, Shield, Wrench, Settings, CheckCircle2,
  Star, AlertTriangle, Award, ChevronRight, MapPin,
} from 'lucide-react';
...
const services = [
  { icon: Wrench, title: 'Spring Replacement', description: 'Torsion & extension spring repair and replacement — same day service available.', image: 'https://media.base44.com/images/public/6940c0d91636ce363ecbf035/7864d5cca_generated_image.png', path: '/services/spring-repair' },
  ...
  { icon: Shield, title: 'Off-Track & Roller Repair', description: 'Rollers replaced and door re-aligned — most jobs completed in under 2 hours.', image: 'https://media.base44.com/images/public/6940c0d91636ce363ecbf035/8f122331d_generated_image.png', path: '/services/roller-replacement' },
  { icon: Award, title: 'New Door Installation', description: 'Modern steel and raised-panel doors — transform your home\'s curb appeal.', image: 'https://media.base44.com/images/public/6940c0d91636ce363ecbf035/42ad8fe28_generated_image.png', path: '/' },
  { icon: CheckCircle2, title: 'Full Tune-Up & Maintenance', description: 'Lubrication, balance check, hardware tightening — extends door life by years.', image: 'https://media.base44.com/images/public/6940c0d91636ce363ecbf035/55372ee69_generated_image.png', path: '/' },
  { icon: AlertTriangle, title: 'Weatherseal & Cable Repair', description: 'Bottom seal, side seals, and cable replacement — keep your garage weatherproofed.', image: 'https://media.base44.com/images/public/6940c0d91636ce363ecbf035/0d4cbef2c_generated_image.png', path: '/services/cable-repair' },
];
    ...
    <div className="min-h-screen bg-white pb-20 lg:pb-0">
      ...
      >
        <div className="container mx-auto px-4 py-16">
          <div className="max-w-2xl">
            <h1 className="text-4xl md:text-6xl font-bold mb-4 leading-tight">Garage Door Repair In New Jersey</h1>
            <p className="text-xl text-blue-100 mb-8">Reliable & Professional Local Garage Door Company</p>
            ...
            <div className="flex gap-4 flex-col sm:flex-row">
              <a href="tel:+12015033118" className="inline-flex items-center gap-3 bg-yellow-500 hover:bg-yellow-400 text-white font-bold text-xl px-10 py-4 rounded-xl transition-colors w-full sm:w-auto justify-center">
                <Phone className="w-6 h-6" /> Give Us A Call
              </a>
              <button
      ...
      <section id="contact-form" className="bg-blue-800 py-12">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-white text-center mb-2">Get a Free Quote — We'll Call You Back Fast!</h2>
          ...
          {formSent ? (
            ...
            <form onSubmit={handleFormSubmit} className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-3">
              <input required type="text" placeholder="Your Name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="rounded-lg px-4 py-3 text-slate-900 text-base outline-none focus:ring-2 focus:ring-yellow-400" />
              <input required type="tel" placeholder="Phone Number" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className="rounded-lg px-4 py-3 text-slate-900 text-base outline-none focus:ring-2 focus:ring-yellow-400" />
              <input required type="text" placeholder="Zip Code" value={formData.zipcode} onChange={(e) => setFormData({ ...formData, zipcode: e.target.value })} className="rounded-lg px-4 py-3 text-slate-900 text-base outline-none focus:ring-2 focus:ring-yellow-400 sm:col-span-2" />
              <textarea placeholder="Message (describe your issue)" value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} rows={3} className="rounded-lg px-4 py-3 text-slate-900 text-base outline-none focus:ring-2 focus:ring-yellow-400 sm:col-span-2 resize-none" />
              <button type="submit" className="bg-yellow-500 hover:bg-yellow-400 text-white font-bold rounded-lg px-6 py-3 text-base flex items-center justify-center gap-2 transition-colors sm:col-span-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((service, index) => (
              <Link key={index} to={service.path} className="block">
                <Card className="overflow-hidden bg-white border border-slate-200 hover:shadow-lg transition-shadow h-full">
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5 max-w-5xl mx-auto">
            {[
              { icon: Award, title: 'Licensed', sub: 'HIC #13VH14019600' },
              { icon: Shield, title: 'Insured', sub: 'Full insurance coverage' },
              { icon: CheckCircle2, title: 'Bonded', sub: 'Surety bond protected' },
              { icon: Star, title: '5-Star Rated', sub: 'Top-rated on Google' },
            ].map((b, i) => (
              <Card key={i} className="text-center bg-white border border-slate-200">
                <CardContent className="p-6">
                  <div className="bg-blue-600 w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-3">
                    <b.icon className="w-6 h-6 text-white" />
                  </div>
                  <h3 className="text-lg font-bold mb-1 text-slate-900">{b.title}</h3>
      ...
      <section className="py-20 bg-blue-800 text-white">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-xl mb-10 text-blue-200 max-w-2xl mx-auto">Call or email to schedule your free on-site assessment</p>
          <div className="flex gap-4 flex-col sm:flex-row justify-center items-center">
            <a href="tel:+12015033118" className="inline-flex items-center gap-3 bg-yellow-500 hover:bg-yellow-400 text-white font-bold text-xl px-12 py-5 rounded-xl transition-colors">
              <Phone className="w-6 h-6" /> Call (201) 503-3118
            </a>
